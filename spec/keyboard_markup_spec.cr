...
describe TelegramBot::ReplyKeyboardMarkup do
  ...
  it "renders text-only buttons as string" do
    buttons = [[
      TelegramBot::KeyboardButton.new("Button 1"),
      TelegramBot::KeyboardButton.new("Button 2")
    ]]

    markup_json = TelegramBot::ReplyKeyboardMarkup.new(buttons).to_json
    JSON.parse(markup_json).should eq({"keyboard" => [["Button 1", "Button 2"]]})
  end
